...
class Color:
    ...
    __slots__ = (
        'name', 'desc_left', 'desc_right', 'hex', 'rgb', 'hsv', 'hls', 'yiq',
        'lch', 'rgb_d', 'hsv_d', 'hls_d', 'yiq_d', 'lch_d', 'dark'
    )
    ...
    def __init__(self,
        ...
        if isinstance(color, str):
            if col_l := color.lower() in Literals.__members__:
                color = Literals.__dict__[col_l].value
            ...
            match mode:
                ...
                case 'lch':
                    ...
                    self.rgb = (
                        gamma(4.0767245293 * l1 - 3.3072168827 * m + 0.2307590544 * s),
                        gamma(-1.2681437731 * l1 + 2.6093323231 * m - 0.3411344290 * s),
                        gamma(-0.0041119885 * l1 - 0.7034763098 * m + 1.7068625689 * s)
                    )
        self.name = name
    ...
    def __getattribute__(self, item):
        try:
            ...
            match item:
                ...
                case 'lch':
                    def gamma_inv(x):
                        return ((x + 0.055) / (1. + 0.055)) ** 2.4 if x >= 0.04045 else x / 12.92
                    r, g, b = (gamma_inv(x) for x in self.rgb)
                    l1 = 0.4121656120 * r + 0.5362752080 * g + 0.0514575653 * b
                    m = 0.2118591070 * r + 0.6807189584 * g + 0.1074065790 * b
                    s = 0.0883097947 * r + 0.2818474174 * g + 0.6302613616 * b
                    l1_ = l1 ** (1. / 3.)
                    m_ = m ** (1. / 3.)
                    s_ = s ** (1. / 3.)
                    L = 0.2104542553 * l1_ + 0.7936177850 * m_ - 0.0040720468 * s_
                    a = 1.9779984951 * l1_ - 2.4285922050 * m_ + 0.4505937099 * s_
                    b = 0.0259040371 * l1_ + 0.7827717662 * m_ - 0.8086757660 * s_
                    C = (a ** 2 + b ** 2) ** (1. / 2.)
                    H = degrees(atan2(b, a))
                    if H < 0:
                        H += 360
